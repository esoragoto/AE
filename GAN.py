    ...
    for epoch in range(n_epoch):
        for x, t in dataloader:
            ...
            if (epoch+1) == 1 or (epoch+1) % 10 == 0:
                print('Epoch:{}/{}, Loss_D: {:.3f}, Loss_G: {:.3f}, D(x): {:.3f}, D(G(z)): {:.3f}/{:.3f}'.format(epoch + 1, n_epoch, errD.item(), errG.item(), D_x, D_G_z1, D_G_z2))
    
    torch.save(generator,     'GAN_G.pth')
    torch.save(discriminator, 'GAN_D.pth')
